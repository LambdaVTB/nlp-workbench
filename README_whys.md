...
Набор новостей за последние n дней —> Из заголовков выделяем Named Entities (названия и имена), и отдельно другие существительные, дополнительно, обучив TF-IDF, на выборке новостей за последний период, выбираем наиболее важные в заголовке 3 слова —> Считаем кол-во NE, кол-во существительных, кол-во значимых существительных получаем самые упоминаемые, а значит популярные места (LOCs), людей (PERs), организации (ORGs),  и вещи (NOUs) за последнее время  —> это и есть тренд
...
***Тренд построенный по словам, определенным TF-IDF, как наиболее важным в заголовках:***
![nouns](vis/example_tfidf.png)
